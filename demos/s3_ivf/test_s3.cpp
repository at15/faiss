...
bool PutObject(const Aws::String& bucketName,
    ...
    *data << content;
    ...
    if (outcome.IsSuccess()) {
        std::cout << "Successfully put object '" << objectKey
                  << "' to bucket '" << bucketName << "'" << std::endl;
        return true;
    } else {
...
int main(int argc, char** argv) {
    if (argc < 4) {
        std::cout << "Usage: " << argv[0]
                  << " <bucket_name> <object_key> <operation> [content]" << std::endl;
        std::cout << "  operation: put or get" << std::endl;
        std::cout << "  content: required for 'put' operation" << std::endl;
        std::cout << std::endl;
        std::cout << "Environment variables:" << std::endl;
        std::cout << "  S3_ENDPOINT_URL - Custom S3 endpoint (e.g., http://localhost:9000 for s3mock)" << std::endl;
        std::cout << "  AWS_EC2_METADATA_DISABLED - Set to 'true' for local testing" << std::endl;
        std::cout << std::endl;
        std::cout << "Example (put): " << argv[0]
                  << " my-bucket test.txt put \"Hello, S3!\"" << std::endl;
        std::cout << "Example (get): " << argv[0]
                  << " my-bucket test.txt get" << std::endl;
        std::cout << std::endl;
        std::cout << "Example (local s3mock):" << std::endl;
        std::cout << "  S3_ENDPOINT_URL=http://localhost:9000 AWS_EC2_METADATA_DISABLED=true \\" << std::endl;
        std::cout << "  " << argv[0] << " test-bucket test.txt put \"Hello!\"" << std::endl;
        return 1;
    }
    ...
    Aws::InitAPI(options);
    {
        ...
        if (operation == "put") {
            if (argc < 5) {
                std::cerr << "Error: Content required for put operation" << std::endl;
